...
{   
    ...
    void Update() 
    {
        if (Input.GetKeyDown(KeyCode.Tab)) //%#Q： 快捷键设置，Ctrl=% Shift=# GetKeyDown(KeyCode) 获取键盘按键按下 强调的是瞬间发生的动作  ,Input.GetKey(KeyCode)键盘按键按住不放,Input.GetKeyUp(KeyCode)键盘按键松开
      {  
          if (GameIsPaused)
         {
